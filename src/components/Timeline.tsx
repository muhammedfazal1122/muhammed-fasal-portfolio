...
import { getMobileAnimationConfig, isMobile } from "@/lib/utils";
...
export default function Timeline() {
  ...
  // on cheap phones just render static timeline without motion wrappers
  if (isMobileDevice) {
    return (
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            <div className="space-y-12">
              {TIMELINE_DATA.map((item, index) => (
                <StaticTimelineItem key={index} item={item} index={index} />
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <section className="relative z-20 bg-[#0a0a0a] min-h-screen py-32 px-4 md:px-12 overflow-hidden" id="journey">
      {/* Background Ambience (disabled on phones) */}
...
// simplified item used only when mobileDevice true
function StaticTimelineItem({ item, index }: { item: any; index: number }) {
  const isEven = index % 2 === 0;
  return (

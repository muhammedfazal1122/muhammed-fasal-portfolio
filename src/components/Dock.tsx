...
const DOCK_ITEMS = [
  ...
  { id: "resume", icon: Icons.FileText, label: "Resume", href: "/MuhammedFasalResume.pdf", external: true },
];
...
const DockIconInner = ({
  ...
  return (
      {/* use plain <a> for external or asset links to avoid Next.js prefetching and RSC fetches */}
      {item.external || item.href.endsWith('.pdf') ? (
        ...
        <Link href={item.href} target={item.external ? "_blank" : undefined} prefetch={false}>
          <motion.div
            ref={ref}
            style={{ width }}
            className="aspect-square w-10 rounded-full bg-gray-700/50 border border-white/10 flex items-center justify-center hover:bg-gray-600/80 transition-colors group relative"
